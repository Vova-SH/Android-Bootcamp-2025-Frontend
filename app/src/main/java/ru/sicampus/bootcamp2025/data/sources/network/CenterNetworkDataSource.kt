...
object CenterNetworkDataSource {
    ...
    suspend fun getCenters(token: String, pageNum: Int, pageSize: Int): Result<CenterListPagingDto> =
        withContext(Dispatchers.IO) {
            runCatching {
                val result = client.get("http://10.0.2.2:9000/api/centers/paginated?page=$pageNum&size=$pageSize") {
                    headers {
